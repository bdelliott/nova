...
class LibvirtDriver(driver.ComputeDriver):
        ...
        if CONF.libvirt.virt_type == "lxc":
            ...
            if CONF.libvirt.uid_maps or CONF.libvirt.gid_maps:
                for device in devices:
                    libvirt_utils.chown_for_id_maps(device.source_dir,
    ...
    def _parse_idmaps(map_string):
        ...
        for mapping in map_string.split(','):
            try:
                guest_id, host_id, count = [int(i) for i in
                                            mapping.split(':')]
                mappings.append((guest_id, host_id, count))
            except ValueError:
    ...
    def get_guest_idmaps(self):
        id_maps = []
        if CONF.libvirt.virt_type == 'lxc':
            if CONF.libvirt.uid_maps:
                uid_mappings = self._parse_idmaps(CONF.libvirt.uid_maps)
                UIDMapClass = vconfig.LibvirtConfigGuestUIDMap
                uid_maps = self._create_idmaps(UIDMapClass, uid_mappings)
                id_maps.extend(uid_maps)
            if CONF.libvirt.gid_maps:
                gid_mappings = self._parse_idmaps(CONF.libvirt.gid_maps)
                GIDMapClass = vconfig.LibvirtConfigGuestGIDMap
                gid_maps = self._create_idmaps(GIDMapClass, gid_mappings)
