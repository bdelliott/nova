...
class _GlanceImageMetaDataCache(object):
    ...
    def remove_entry(self):
        ...
        LOG.debug("Removed metadata for image '%(image_id)s' from "
    ...
    def get_image_meta(self, image_id):
        ...
        if (recheck_age and
            ...
            LOG.debug("Forcing re-check of metadata for image "
                      "'%(image_id)s' from cache", locals())
            return
        self.update_entry(entry)
        LOG.debug("Returning metadata for image '%(image_id)s' from "
                  "cache", locals())
    ...
    def store_image_meta(self, image_meta):
        ...
        if image_id in self.entries_by_id:
            ...
            LOG.debug("Updating metadata for image '%(image_id)s' in "
                      "cache", locals())
            entry = self.entries_by_id[image_id]
        ...
        rough_size = self.rough_size
        num = len(self.entries_list)
        LOG.debug("Stored metadata for image '%(image_id)s' in "
...
class GlanceClientWrapper(object):
    ...
    def _create_onetime_client(self, context, version):
        ...
        if context.glance_api_servers:
            ...
            LOG.debug("Using client specified Glance API server: %(host)s:"
