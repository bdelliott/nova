...
"""The Scheduled Images extension.
    spec: https://wiki.openstack.org/wiki/Nova-scheduled-images-extension .
"""
...
class ScheduledImagesController(wsgi.Controller):
    ...
    def _delete_schedules(self, server_id, schedules):
        """Makes requests to QonoS API to delete Image Schedules."""
        if len(schedules) == 0:
    ...
    def delete(self, req, server_id):
        ...
        try:
            ...
            self._delete_schedules(server_id, schedules)
    ...
    def _create_image_schedule(self, req, server_id):
        """Makes requests to QonoS API using QonoS client to create
           Image Schedule and cleans up unnecessary extra schedules.

           The wiki says to update the schedule if one already exists.
           Since the schedule is random, we can keep one entry and delete
           all the other ones from past race conditions. Essentially, it's
           a noop for update on the Image Schedule.
        """
        context = req.environ['nova.context']
        tenant = context.project_name
        ...
        try:
            ...
            if len(schedules) == 0:
                ...
                self._delete_schedules(server_id, schedules[1:])
        except qonos_exc.ConnRefused:
            LOG.warn(_('QonoS API unreachable.'))
            raise exc.HTTPInternalServerError()

    def is_valid_body(self, body):
        """Validates the Image Schedule request body."""
...
class ScheduledImagesFilterController(wsgi.Controller):
    """Controller class extending servers core controller for Scheduled
       Images.
    """
    ...
    def _get_meta_from_cache(self, req, server_id):
        ...
        meta = nova_utils.instance_sys_meta(instance)
    ...
    def delete(self, req, resp_obj, id):
        ...
        if resp_obj.code == 204 and authorize_filter(context):
            ...
            if si_meta_str:
                ...
                # NOTE(jculp): do not delete schedules when deleting an
                # instance to reduce load on qonos api. deletes will be
                # handled upon execution of a subsequent job for this
                # instance by qonos.


class Scheduled_images(extensions.ExtensionDescriptor):
    """Enables automatic scheduled images to be taken of a server."""
