...
class ServicesController(object):
    ...
    def index(self, req):
        ...
        services = self.host_api.service_get_all(context)
        return {"services": [_build_service(base_url, s) for s in services]}

    def _safe_service_get(self, context, id):
        try:
            return self.host_api.service_get(context, id)
...
class Rax_services(extensions.ExtensionDescriptor):
    """Services support."""
