...
class Client(object):
    ...
    def do_request(self, method, action, body=None,
        ...
        try:
            ...
            if status_code in (httplib.OK,
                ...
                msg = (_("Server %(status_code)s error: %(data)s") %
                       {'status_code': status_code, 'data': data})
                raise QuantumServerException(msg)
    ...
    def update_port(self, network, port, body=None):
        """Updates a port on a given network."""
        body = self.serialize(body)
        return self.do_request("PUT", self.port_path % (network, port),
                               body=body)
