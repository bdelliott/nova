...
from oslo import messaging as messaging
...
quantum2_opts = [
    ...
    cfg.BoolOpt('rs_dual_mode',
                default=False,
                help='Enable Dual Mode networking'),
    cfg.StrOpt('rs_bridge_public',
               default='xenbr0',
               help='Public net bridge (xenbr0)'),
    cfg.StrOpt('rs_bridge_service',
               default='xenbr1',
               help='Service net bridge (xenbr1)'),
    cfg.FloatOpt('rxtx_base', default=1.0,
                 help='Base RXTX for all rxtx_cap calculations in mbit')
]
...
class QuantumManager(manager.Manager):
    ...
    target = messaging.Target(version='1.9')
    ...
    def _get_nw_map(self):
        ...
        for nw in networks:
            ...
            if CONF.rs_dual_mode:
                if nw['label'] == 'public':
                    self._nw_bridge_map[global_uuid] = CONF.rs_bridge_public
                elif nw['label'] == 'private':
                    self._nw_bridge_map[global_uuid] = CONF.rs_bridge_service
    ...
    def _deallocate_port(self, tenant_id, network_id, interface_id,
                         port_id=None):
        if CONF.rs_dual_mode and self._nw_map.get(network_id):
            return
    ...
    def _map_network(self, network_id, label):
        ...
        kwargs = {'nvp_managed': True}
        # NOTE(jhammond) If network_id gets mapped, store original in meta
        if mapped_id != network_id:
            kwargs["original_id"] = network_id
            if CONF.rs_dual_mode:
                if network_id in self._nw_bridge_map:
                    kwargs['bridge'] = self._nw_bridge_map[network_id]
                    kwargs['nvp_managed'] = False

                elif mapped_id in self._nw_bridge_map:
                    kwargs['bridge'] = self._nw_bridge_map[mapped_id]
                    kwargs['nvp_managed'] = False

        return model.Network(id=mapped_id, label=label, **kwargs)

    def _vif_from_network(self, m_vif, network_id, label, rxtx_factor=None,
                          rackconnect=False):
    ...
    def get_vifs_by_instance(self, context, instance_id):
        ...
        return self._vifs_to_model(vifs, rackconnect=rackconnect)
    ...
    def deallocate_interface_for_instance(self, context, instance_id,
        ...
        self.m_conn.deallocate_interface_for_instance(context.project_id,
                                                      instance_id,
                                                      interface_id)
        return self._vifs_to_model([vif])
    ...
    def allocate_interface_for_instance(self, context, instance_id,
        ...
        return self._vifs_to_model([vif], rxtx_factor=rxtx_factor,
                                   rackconnect=rackconnect)
    ...
    def allocate_for_instance(self, context, instance_id, rxtx_factor,
                              ...
                              **kwargs):
        LOG.debug('network allocations for instance %s', instance_id)
        ...
        nw_info = self._vifs_to_model(vifs, rxtx_factor=rxtx_factor,
                                      rackconnect=rackconnect)
    ...
    def _update_port_allowed_address_pairs(self, tenant_id, instance_id,
        ...
        """
        # NOTE(tr3buchet): allowed address pairs only impact rs_networks which
        #                  are no longer in quantum in dual mode. As a result,
        #                  we never update allowed address pairs in dual mode
        if CONF.rs_dual_mode:
            return
    ...
    def add_fixed_ip_to_instance(self, context, instance_id, host, network_id,
        ...
        if not context.is_admin:
            LOG.error(_('Must be admin context to add fixed ip'))
            return
        LOG.debug('adding fixed_ip to instance |%(instance_id)s| on '
    ...
    def remove_fixed_ip_from_instance(self, context, instance_id,
                                      host, address, **kwargs):
        if not context.is_admin:
            LOG.error(_('Must be admin context to remove fixed ip'))
            return
        LOG.debug('removing fixed_ip |%(address)s| from '
                  'instance |%(instance_id)s|',
                  {'address': address, 'instance_id': instance_id})
    ...
    def create_networks(self, context, label, cidr, gateway=None, **kwargs):
        ...
        LOG.debug(_log_kwargs('Network created in quantum', **extra),
                  extra=extra)
    ...
    def deallocate_for_instance(self, context, instance, **kwargs):
        ...
        self.m_conn.allocate_for_instance_networks(tenant_id, instance['uuid'],
                                                   [])
        ...
        for vif in vifs:
            ...
            try:
                ...
                extra = {'instance_id': instance['uuid'],
    ...
    def delete_network(self, context, uuid, **kwargs):
        ...
        try:
            ...
            LOG.debug('Deleting network %(network_id)s for tenant '
    ...
    def get_instance_nw_info(self, context, instance_id, project_id,
        ...
        try:
            vifs = self.m_conn.get_allocated_networks(instance_id)
            LOG.debug('Got VIFs from melange for instance '
                      '(%(instance_id)s): %(vifs)s',
                      {'instance_id': instance_id, 'vifs': vifs})

            self._set_vif_id_on_device(vifs)
            ...
            with excutils.save_and_reraise_exception():
                msg = _('Failed to get nw_info!!! for instance '
                        '|%s|') % instance_id
                LOG.exception(msg)
        LOG.debug('NWInfo for instance (%(instance_id)s): %(nw_info)s',
                  {'instance_id': instance_id, 'nw_info': nw_info})
        return nw_info

    def _set_vif_id_on_device(self, vifs):
        vifs_missing_vif_id_on_device = \
            [vif for vif in vifs if not vif['vif_id_on_device']]

        for vif in vifs_missing_vif_id_on_device:
            tenant_id = None
            network_id = None
            port_id = None
            ...
            # NOTE(jmeridth) Don't query Quantum/NVP when the network is
            #                publicnet or servicenet.  Only run for
            #                isolated networks
            if network_id in self._nw_map:
                return

            try:
                port_id = self.q_conn.\
                    get_port_by_attachment(tenant_id,
                                           network_id,
                                           vif['id'])
            except Exception:
                with excutils.save_and_reraise_exception():
                    msg = _('Error getting port id for '
                            'vif: %(id)s')
                    LOG.exception(msg % vif['id'])

            if port_id:
                try:
                    self.m_conn.\
                        set_interface_vif_id_on_device(vif['id'], port_id)
                except Exception:
                    msg = _('Could not set vif_id_on_device for vif: %(id)s')
                    LOG.exception(msg % vif)
