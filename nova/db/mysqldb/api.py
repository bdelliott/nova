        ...
        def inner(*args, **kwargs):
            ...
            while True:
                try:
                    ...
                    if not retry:
                        raise
                    LOG.warning(_("Will retry DB API call '%(name)s' due to "
                                "exception: %(e)s"),
...
class API(object):
    def __init__(self):
        ...
        LOG.debug("MySQLDB API instantiated.")
    ...
    @dbapi_method()
    def instance_get_by_uuid(self, ctxt, uuid, columns_to_join=None,
                             use_slave=False):
        with self.pool.get() as conn:
            instance = models.Models.Instance.get_by_uuid(
                    conn, ctxt, uuid,
                    columns_to_join=columns_to_join)
        return instance.to_dict()

    @dbapi_method()
    def instance_get_all(self, ctxt, columns_to_join=None):
        with self.pool.get() as conn:
