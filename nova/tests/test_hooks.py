...
class SampleHookA(hooks.BaseHook):
    name = "a"
...
class MockEntryPoint(pkg_resources.EntryPoint):
    ...
    def __init__(self, hook_cls):
        self.hook_cls = hook_cls
        super(MockEntryPoint, self).__init__('test_hook', 'a.module')

    def load(self):
        return self.hook_cls
...
class HookTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        def mock_iter_ep(namespace, name=None):
            for cls in [SampleHookA, SampleHookB]:
                yield MockEntryPoint(cls)

        self.stubs.Set(pkg_resources, 'iter_entry_points', mock_iter_ep)
    ...
    def test_order_of_execution(self):
        ...
        self.assertEqual(['prea', 'preb', 'postb', 'posta'], called_order)
