...
class QuantumPrimeManagerInterfaceTests(test.TestCase):
    ...
    def test_deallocate_for_instance(self):
        ...
        self.stubs.Set(melange_connection.MelangeConnection,
                       'allocate_for_instance_networks', dummy_list)
        instance = {'uuid': str(uuid.uuid4()), 'project_id': 2}
...
class Quantum2ManagerTestRackConnect(test.TestCase):
    ...
    def test_vifs_haz_rackconnect(self):
        with contextlib.nested(
            mock.patch(self.q_client + '.create_and_attach_port'),
            mock.patch(self.m_client + '.get_networks_for_tenant'),
            mock.patch(self.m_client + '.allocate_for_instance_networks'),
            ...
            nw_info = net_manager.allocate_for_instance(self.context,
                                                        instance_id=1,
                                                        rxtx_factor=1,
                                                        project_id='project1',
                                                        host='host')
            for vif in nw_info:
                if vif['network']['label'] == self.servicenet_label:
                    self.assertTrue(vif.get_meta('rackconnect'))
...
class Quantum2ManagerGetIpsAndIdsFromVifs(test.TestCase):
    def test_get_ips_and_ids_from_vifs(self):
        ...
        self.assertEqual(addresses, ['10.0.0.100'])
        self.assertEqual(tenants, set(['project1']))
        self.assertEqual(network_uuids, set([network_uuid]))
        self.assertEqual(network_names, set([network_name]))
...
class Quantum2ManagerGenerateAddressPairs(test.TestCase):
    def test_generate_address_pairs(self):
        ...
        self.assertEqual(res[0]['ip_address'], '10.0.0.100')
        self.assertEqual(res[0]['mac_address'], '00:00:00:00:00:00')
...
class Quantum2ManagerDeallocatePort(test.TestCase):
    ...
    def test_deallocate_port_should_use_the_port_passed_in(self):
        _port_id = 8033
        quantum_client_connection = quantum_connection.QuantumClientConnection
        quantum_client_connection.detach_and_delete_port = mock.MagicMock()
        quantum_client_connection.get_port_by_attachment = mock.MagicMock()
        ...
        self.net_manager._deallocate_port(self.tenant_id,
                                          ...
                                          port_id=_port_id)
        self.assertEqual(
            0, quantum_client_connection.get_port_by_attachment.call_count)
        quantum_client_connection.detach_and_delete_port.assert_called_with(
            self.tenant_id, self.network_uuid, _port_id)
...
class Quantum2ManagerVifFromNetwork(test.TestCase):
    ...
    def test_vif_from_network(self):
        ...
        res = self.net_manager._vif_from_network(vifs, self.network_uuid,
                                                 self.label)
        self.assertEqual(res['network']['subnets'][0]['ips'][0]['address'],
                         '10.0.0.100')
...
class Quantum2ManagerVifsToModel(test.TestCase):
    ...
    def test_vifs_to_model(self):
        ...
        self.assertEqual(res[0]['network']['subnets'][0]['ips'][0]['address'],
                         '10.0.0.100')
...
class Quantum2GetInstanceUUIDS(test.TestCase):
    ...
    def test_get_instance_uuids_by_ip_filter(self):
        ...
        uuids = self.net_manager.get_instance_uuids_by_ip_filter(self.context,
                                                                 filters)
        self.assertEqual(uuids, [{'instance_uuid': 'instance_uuid'}])
...
class Quantum2VirtualInterfaces(test.TestCase):
    ...
    def test_create_virtual_interface(self):
        ...
        self.stubs.Set(melange_connection.MelangeConnection,
                       ...
                       self._dummy)
        self.stubs.Set(manager.QuantumManager, "_vifs_to_model", self._dummy)
    ...
    def test_delete_virtual_interface(self):
        ...
        network_id = uuid.uuid4()
        vif = _vif_helper(project_id, network_id)
        vif_tenant_id = vif['ip_addresses'][0]['ip_block']['tenant_id']
        vif_network_id = vif['ip_addresses'][0]['ip_block']['network_id']
        vif_interface_id = vif['id']
        vif_port_id = vif['vif_id_on_device']

        manager.QuantumManager._deallocate_port = mock.MagicMock()
        _quantum_deallocate_port = manager.QuantumManager._deallocate_port
        ...
        with mock.patch("nova.network.quantum2.melange_connection."
            ...
            self.net_manager.deallocate_interface_for_instance(
                self.context, instance_id, vif_interface_id)

            self.assertEqual(patch.called, True)

            _quantum_deallocate_port.assert_called_with(vif_tenant_id,
                                                        vif_network_id,
                                                        vif_interface_id,
                                                        port_id=vif_port_id)
