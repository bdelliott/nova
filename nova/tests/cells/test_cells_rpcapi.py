...
class CellsAPITestCase(test.TestCase):
    ...
    def test_bdm_create(self):
        fake_bdm = {'id': 2, 'other': 'meow'}

        call_info = self._stub_rpc_method('cast', None)

        self.cells_rpcapi.bdm_create(self.fake_context, fake_bdm)

        expected_args = {'bdm': fake_bdm}
        self._check_result(call_info, 'bdm_create',
                expected_args, version='1.5')

    def test_bdm_update(self):
        fake_bdm = {'id': 2, 'other': 'meow'}

        call_info = self._stub_rpc_method('cast', None)

        self.cells_rpcapi.bdm_update(self.fake_context, fake_bdm,
                                     create='fake-create')

        expected_args = {'bdm': fake_bdm, 'create': 'fake-create'}
        self._check_result(call_info, 'bdm_update',
                expected_args, version='1.5')

    def test_bdm_destroy(self):
        call_info = self._stub_rpc_method('cast', None)

        self.cells_rpcapi.bdm_destroy(self.fake_context,
                                      'fake-uuid',
