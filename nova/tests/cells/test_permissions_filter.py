...
class MockConfig(cells_cfg.CellsConfig):
    def _reload_cells_config(self):
        c1_full_name = utils._PATH_CELL_SEP.join(["US", "East",
            CELL_NAMES[0]])
        cell1_rules = [
            ('somerole', 'allow', 'b'),
        ]

        c2_full_name = utils._PATH_CELL_SEP.join(["US", "East",
            CELL_NAMES[1]])
        cell2_rules = [
            ('somerole', 'deny', 'b'),
        ]

        c3_full_name = utils._PATH_CELL_SEP.join(["US", "East",
            CELL_NAMES[2]])
        cell3_rules = [
            ('bofh', 'deny', 'b'),
            ('otherrole', 'allow', 'b'),
...
class PermissionsFilterTest(test.TestCase):
    ...
    def setUp(self):
        ...
        my_cell_name = utils._PATH_CELL_SEP.join(["US", "East", "c2"])
        ...
        routing_path = utils._PATH_CELL_SEP.join(["US", "East"])
    ...
    def testDefaultSingleCell(self):
        c1_full_name = utils._PATH_CELL_SEP.join(["US", "East", CELL_NAMES[0]])
