...
class ScheduledImagesTest(test.TestCase):
    ...
    def test_delete_image_schedule_no_schedules(self):
        # Need an empty schedule list for this test
...
class ScheduledImagesFilterTest(test.TestCase):
    ...
    def assertScheduledImages(self, dict_, value, is_present=True):
        if is_present:
            self.assertTrue(OS_SI in dict_)
            self.assertTrue('retention' in dict_[OS_SI])
            self.assertEqual(dict_[OS_SI]['retention'], value)
        else:
            self.assertTrue(OS_SI not in dict_)
    ...
    def test_index_servers_with_true_query(self):
        ...
        for server in servers:
            self.assertScheduledImages(server, 7, is_present=False)
